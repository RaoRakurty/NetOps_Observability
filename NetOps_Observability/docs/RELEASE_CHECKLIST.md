...
| | Step | How | State |
|---|---|---|---|
| 3.1 | **Reference-capacity regression.** `t-storm-2.5k`, seed 20260829, scorer v2: completion ≤ 2 700 s, losslessness (injected == persisted, 0 DLQ), memory caps held with no sustained growth, accuracy ≥ 93 %, replica stability, exact aggregation accounting. Verdict is three-valued — `INVALID` can never be reported as `PASS`. | `python3 scripts/release-qualify.py` (runbook: [`docs/runbooks/release-qualification.md`](runbooks/release-qualification.md)) — ~1 h exclusive rig time, ≥ 10 GiB free, `load1 ≤ 6.0`, exactly 2 correlation replicas | 🟡 MANUAL — **and never yet executed**: the suite ships and self-tests, but the first leg is owner-gated rig time |
| 3.1a | **The qualification suite proves its own logic** — the grader is re-graded against the checked-in `storm-s11` leg fixture and mutations of it (a regressed gate, accuracy under the floor, scorer v1, aggregation that does not close, a replica restarted mid-run) must each be CAUGHT. Not qualification evidence: it measures a fixture, not a build. | `python3 scripts/release-qualify.py --self-test` — seconds, no rig, no stack, no `.env` | 🟢 AUTOMATED (runs in `pytest tests/`) |
