...
| Item | Status | Notes |
|---|---|---|
| 72h soak #2 (GA-candidate build, 1K) | ⚠️ | **COMPLETED full 72h (no abort — a first).** Core gates PASS (completion 0-pending/4260 cohorts, drain peak-lag-4, **memflat clean = leak resolved**). **FINAL: 6 PASS / 3 FAIL — all 3 FAILs non-product artifacts:** accounting (6.3M events — Fable's disk-crisis Kafka-retention cap, self-inflicted, reverted) + stability (grader couldn't read 1 of 2 replica logs; **real signal clean — restarts=0, 72h uptime**) + cleanup (purge timed out on 19.2M docs — **NOW REMEDIATED**: async purge done, 0 docs left, force-merge reclaimed disk to 85%/12G free). Verdict: SOAK2_VERDICT_2026-08-27.md |
| ✅ DECISION: (A) ACCEPT (owner 2026-08-27) | ✅ | Soak accepted as GA proof — 1K sustained 72h, memory-stable, correlation current. Accounting losslessness = documented gap, closeable via a SHORT accounting run (not another 72h). Deploy hold LIFTED. |
| Post-soak deploy batch | ✅ DONE (incident resolved) | api rebuilt (BGP routes LIVE — /api/bgp/watchlist 401=auth-gated), gnmic per-VRF wildcard LIVE, TLS :443→200, all 27 svcs healthy. **Surfaced 2 latent bugs on api recreate:** (1) device-store boot walked 38,666 synthetic tombstones sequentially → 6-min wedge (FileKV.LoadPrefix O(N)); FIXED `6b79ea58` (FileKV.LoadPrefix parallelized, 50k regression test, gate-clean), api rebuilt+redeployed (boot ~3s), verified. Growth follow-up = TRACKER #175. (2) gnmic.yaml YAML error from cd146163 (spine1 dedent) → FIXED+committed. healthcheck→:8094 still pending (Go image has no python for the compose test). |
| ⚠️ Tombstone cleanup mis-exec | noted | archived-then-deleted 38,666 synthetic tombstones, but the tar glob-before-`-C` produced an EMPTY archive → files unrecoverable. Impact negligible (synthetic soak devices; SNMP-discovery off, 0 manual devices, 0 tombstones now). Feedback logged. |
| miniladder cron re-enable | ✅ | re-enabled (both lines) after the boot-walk fix deployed |
