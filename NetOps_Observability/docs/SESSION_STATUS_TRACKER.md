...
| Post-soak deploy batch | ⚠️ DONE w/ incident | api rebuilt (BGP routes LIVE — /api/bgp/watchlist 401=auth-gated), gnmic per-VRF wildcard LIVE, TLS :443→200, all 27 svcs healthy. **Surfaced 2 latent bugs on api recreate:** (1) device-store boot walked 38,666 synthetic tombstones sequentially → 6-min wedge (FileKV.LoadPrefix O(N)); triaged by removing them, **code fix building (Opus)** then redeploy. (2) gnmic.yaml YAML error from cd146163 (spine1 dedent) → FIXED+committed. healthcheck→:8094 still pending (Go image has no python for the compose test). |
...
| miniladder cron re-enable | ⏸ HELD | stays suspended until the boot-walk fix lands+redeploys (cron churns 1K devices/run → regenerates tombstones) |
