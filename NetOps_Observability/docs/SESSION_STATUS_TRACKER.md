...
| Item | Status | Notes |
|---|---|---|
| 72h soak #2 (GA-candidate build, 1K) | ⚠️ | **COMPLETED full 72h (no abort — a first).** Core gates PASS (completion 0-pending/4260 cohorts, drain peak-lag-4, **memflat clean = leak resolved**). **2 gate FAILs, both non-product artifacts:** accounting (6.3M events — Fable's disk-crisis Kafka-retention cap, self-inflicted, reverted) + stability (grader couldn't read a 3-day replica log; **real signal clean — both replicas restarts=0, 72h uptime**). Verdict: SOAK2_VERDICT_2026-08-27.md |
