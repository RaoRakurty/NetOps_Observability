...
| 20 | **Multi-tenant telemetry isolation** — tenant-tag at Vector ingest + ClickHouse row policies + OpenSearch per-tenant indices/DLS + VM tenants | 🟡 **Phase 1 started (PARKED, uncommitted)** | Design doc `docs/design/multitenant-telemetry-isolation.md` (decision: operator-run receivers + device→tenant attribution, model B). Phase 1 = ingest tenant-tagging: Go `telemetry_enrichment.go` (device→tenant CSV exporter, tests green) + `startTenantEnrichment` hook done; REMAINING in Phase 1 = Vector aggregator enrichment-table stamping, CH/OS `tenant_id` columns, correlation stamping, compose wiring. Reads stay on the working device-matcher (Phase 1 is additive groundwork); Phase 2 = DB-enforced reads (row policies/DLS). Real customer data separation for flows/logs/metrics. Independent of #19. |
