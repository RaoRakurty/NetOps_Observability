...
| 129 | **Sealed Fields (reversible masking) — finish the vertical.** Commits 1–3 SHIPPED 2026-07-31: `sealing` package (CryptoProvider over the vault's envelope model, `<enc:v1:…>` token, edge VRL generator, Vector↔Go cipher parity proven against the pinned binary — this is where Vector's CTR was found to be Ctr64LE, not Go's Ctr128BE) + the `seal` action wired into the processor framework behind the `SealEngine` seam. OPEN: (a) **vault-backed `KeyProvider`** — the vault has per-tenant DEKs but NO key versioning, so `TenantKeyVersion`/`RotateTenant` need a versioned wrapped-key format + migration; until it lands `SealEngine` is nil in production and seal refuses validation by design; (b) router **secret backend** serving `cxseal.<tenant>`/`cxmac.<tenant>` via Vector `exec` (mechanism already verified working); (c) `POST /api/pipeline/processors/unseal` + RBAC `sensitive_data.unmask`/`view_masked`/`manage_rules`; (d) audit records + `unmask_requests_total` metrics; (e) UI reveal flow + Sensitive Data Access Audit page; (f) docs. | High | 🟡 crypto floor + edge parity + action SHIPPED; key custody next |
