...
| # | Item | Pri | Status |
|---|------|-----|--------|
| 152 | **GA scale programme — network digital twin (simulator).** Approved GA plan §3 (2026-08-16): multi-tenant device simulator (SNMPv3/v2c via snmpsim-class engines, gNMI + NETCONF endpoints, syslog with real mnemonics, IPFIX) plus cloud/ISP seam stories (AWS DX, Azure ExpressRoute, GCP Interconnect, carrier flaps), tenant-mapped accounts, scenario DSL, clean-slate reset. Purpose: labeled ground truth for RCA-accuracy SLO + drives the L2+ ladder. Scaled-down single-host mode first; full scale needs simulator VMs (plan §4 infra table — note it maps Redpanda/Redis→Kafka/Valkey in our stack). | High | ⏳ not started — hardware/VM-gated; design can start any time |
