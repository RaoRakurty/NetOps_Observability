...
> Branch: `feat/observability-platform` · Last updated: 2026-06-03
...
| # | Task | Pri | Status |
|---|------|-----|--------|
| 15 | **Phase 5 — PostgreSQL RLS** (app-state) | High | ✅ **substantially landed.** All app-state tables `ENABLE/FORCE ROW LEVEL SECURITY` + `tenant_iso` policy (migrations 0001-0003); `withTenant` binds the `app.current_tenant` GUC per transaction; `assertRLSCapable` refuses a BYPASSRLS/superuser role (fail-closed). **Per-request enforcement** active for the converted/new per-row repositories (#33 users + saved_objects, #32 audit, report queue/exec/deliveries). Remaining blob-bridge stores (api_keys/snmp_credentials/singletons) read platform-scoped with RLS backstopping writes — intentional (bounded config, see #33). |
