...
| 147 | **Code-lightening programme (owner ask 2026-08-03).** Full-stack audit COMPLETE — `docs/audit/lightening-audit-2026-08-03.md` (evidence-verified, tranched). Headline: frontend ships a 4.6 MB initial JS bundle with zero route splitting (~3.1–3.4 MB removable); backend has ~875 LOC compiler-verifiable dead code + ~2,000 LOC owner-gated dormant engines (`segclass` ~908, `portintel` analysis half ~630, `appid`/`cloud` built-ahead lanes ~320 — wire-or-retire decisions needed) + ~550 LOC dedup. Execute as tranches T1–T6 per the audit's synthesis table; T5 blocked on owner wire-or-retire calls; wireless dormant code stays (Phase-9 HOLD). `log/slog` migration and no-utils-helper consolidation assessed and REJECTED — do not re-propose. **Owner decisions 2026-08-03:** T1–T4 GO ("just make sure we are not breaking anything"); T5: `segclass` → RETIRE (execute with the backend tranche); `portintel` scoring → KEEP (wiring planned); `appid`/`cloud` lanes → KEEP (owner: "not sure if I need it, but keep it" — revisit when the RCA app-identity lane firms up or at the next audit; do not re-flag as junk meanwhile). | High | ⏳ executing T1–T4 + segclass retirement |
| 146 | **SSO users: correct per-user roles + user settings parity (owner ask 2026-08-03).** Federated users must land with (a) the RIGHT role per user — today `RoleFor()` maps Keycloak realm-roles/groups but the lab grants roles per-user in Keycloak; production path is Okta group → Keycloak IdP mapper (*Advanced Claim to Role*) → `OIDC_ADMIN_ROLES`/`OIDC_OPERATOR_ROLES`, needs productizing + docs; and (b) the same user-settings experience as local users — idle/absolute timeout comes from `sessionPolicy(tenant, role)` (already applies to SSO logins via `mintSession` — VERIFY, don't assume), login-screen theme is chosen at login + persisted per account (verify `UpsertFederated`-provisioned accounts keep appearance/preferences across logins and that MergeFederated doesn't clobber them), plus whatever else lives in per-user settings. Scope first: enumerate the user-settings surface, test each against a federated account, fix the gaps, ship with regression tests. | High | open — queued after the code-lightening pass (owner order) |
