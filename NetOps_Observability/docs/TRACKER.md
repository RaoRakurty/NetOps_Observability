...
| # | Item | Pri | Status |
|---|------|-----|--------|
| 154 | **Engine ranking/folding misses measured by twin ground truth (first RCA-accuracy baseline).** Live run 081700325h7x (2026-08-17), DX-flap story: object detected/tier/affected/seam-grounded all ✓, but (a) top hypothesis picked `sig.ent.middle-mile.dia-egress-corroborated` (owner isp) over `private-interconnect-bgp-down` (owner carrier) at EQUAL confidence 1.0 — tie-breaking ignores seam-type evidence (the grounding edge was a DX seam); (b) a 3-node cloud-only object stood apart instead of folding into the incident (merge/continuation gap across the seam weld). Evidence: data/twin/20260817T003251Z-081700325h7x/accuracy-report.{json,md} + objects 2afb852e/e935a865. Fix = ranking tie-break by grounded-seam type + cross-seam fold rule; re-measure with the same seeded story. | Medium | ⏳ measured + evidenced, not started |
| 152 | **GA scale programme — network digital twin (simulator).** Approved GA plan §3 (2026-08-16): multi-tenant device simulator (SNMPv3/v2c via snmpsim-class engines, gNMI + NETCONF endpoints, syslog with real mnemonics, IPFIX) plus cloud/ISP seam stories (AWS DX, Azure ExpressRoute, GCP Interconnect, carrier flaps), tenant-mapped accounts, scenario DSL, clean-slate reset. Purpose: labeled ground truth for RCA-accuracy SLO + drives the L2+ ladder. Scaled-down single-host mode first; full scale needs simulator VMs (plan §4 infra table — note it maps Redpanda/Redis→Kafka/Valkey in our stack). | High | 🟡 design SHIPPED (docs/design/network-digital-twin.md) + **T1 core SHIPPED 2026-08-17** (scripts/lab/twin/: DSL, seeded fault stories w/ ground truth, accuracy scorer, partition-spread proof; live run 081700325h7x: G-1 spread PASS ±20% across 2 replicas, G-2 baseline 50%). REMAINING: fidelity wave (snmpsim/traps/IPFIX/gNMI/twinnet source-ip overlay, compose overlay, mini-ladder composition) + T2 rig scale |
