...
| 40 | **ITSM connectors — admin-UI config (un-grey ServiceNow/Jira)** — the incident→ITSM projection (#38) is fully wired but `notify/{servicenow,jira}` are **env-only** (`FEATURE_*_NOTIFICATIONS` + creds), built in `newServer()` at startup. The Integrations UI shows them **greyed** (no config form), so unless the operator sets env vars `itsmConfigured()` is false and incidents never project. Build a UI config store+form (mirror the SMTP/OIDC pattern) so ServiceNow/Jira are configurable without a restart. | High | ⏳ open — *why incidents show no ITSM link today* |
