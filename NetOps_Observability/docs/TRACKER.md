...
| 149 | **API metrics scrape dark since the mTLS milestone.** The `netops-api` scrape job (`src/config/vmscrape.yml:22`, plaintext `http://api:8080/metrics`) has been `up==0` since ~04:45 2026-08-04 — the moment nginx→API mutual TLS shipped and the API began answering 8080 as HTTPS+mTLS (verified: `tlast_change_over_time(up{instance="api:8080"}[48h])`, and a plaintext probe now gets the Go TLS server's "400 Bad Request"). All `netops_*` API metrics are missing and `ScrapeTargetDown` (critical) fires continuously. Fix per the mTLS design: issue a VictoriaMetrics client SVID from the internal CA (`data/tls/` holds only api/nginx/ca today), mount it into the victoria container, and move the job to `scheme: https` + `tls_config` (ca_file/cert_file/key_file) — do NOT re-expose /metrics plaintext. Found 2026-08-04 while resolving the unseal outage. | High | ⏳ scoped, not started |
