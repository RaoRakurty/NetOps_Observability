...
| 129 | **Sealed Fields (reversible masking) — remaining.** SHIPPED 2026-07-31: `sealing` package (CryptoProvider over the vault envelope model, `<enc:v1:…>` token, edge VRL generator, Vector↔Go cipher parity vs the pinned binary — where Vector's CTR was found to be Ctr64LE not Go's Ctr128BE), versioned per-tenant DEKs in the vault (additive; v1 IS the legacy key), the `seal` action, and the full reveal path (`POST /api/pipeline/processors/unseal`, `sensitive_data` RBAC module, audit with token fingerprints + no plaintext, `netops_unmask_requests_total`, reveal UI, `docs/design/sealed-fields.md`). OPEN: (a) **router secret backend** serving `cxseal.<tenant>`/`cxmac.<tenant>` via Vector `exec` — the mechanism is verified working but the endpoint + backend script are not written, so the EDGE cannot seal yet (in-process sealing and the whole reveal path do work); (b) a **Sensitive Data Access Audit** page filtering the trail to unseal events; (c) key-rotation UI/runbook. | High | 🟡 crypto + custody + action + reveal SHIPPED; edge key delivery next |
