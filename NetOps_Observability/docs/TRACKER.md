...
| 151 | **TLS completion programme (owner-ordered 2026-08-04, four steps IN ORDER).** Authority for "agreed architecture" = the `docs/security/` corpus (`CORRELIX_SECURITY_IMPLEMENTATION_BACKLOG.md` SEC-001…024 with owner steer §0a, HLD/LLD, `CORRELIX_SECURITY_SOURCE_ALIGNMENT.md` vs the owner's TLS-Cloudnative design) + `docs/design/tls-architecture.md` + `transport-encryption-2026-08-04.md`. **Step 1 — implement the gaps:** as-built delta audit (SEC-001 shape; known 2026-08-04: SEC-002 validator, SEC-004/005 ingress+nginx→api mTLS, SEC-003.1 seal-gate, victoria scrape SVID are LIVE; open = SEC-003.2/.3 → SEC-006/007 Kafka mTLS+ACLs, SEC-008–012 five stores, SEC-013 Vector ingestion, SEC-018 seal transport/REQUIRE_SEAL, SEC-019.1 rotation, SEC-020/021 posture observability+UI), then execute epic-by-epic per the backlog DAG — one component per step, stack green between steps, boot validator trending fatal→0, THE LAB CONVERTS EPIC-BY-EPIC as each lands (owner 2026-08-04: the lab is the TLS testbed step 2 will exercise), protocols that cannot encrypt become DECLARED plaintext (transport-encryption P4), never silent. **Step 2 — execute the phased assurance test plan** (the owner's 13-phase spec of 2026-08-04: edge contracts `mtls-edges.yaml` + `telemetry-lanes.yaml`, coverage matrix, static+runtime identity checks, negative-cert matrix, lane canaries, count reconciliation, restart/fault suite incl. the five pinned regression classes, cert+CA lifecycle, tenant/service authz matrix, perf/scale, CI gates ≈ SEC-023 superset) and PRESENT THE REPORT with findings ranked P0–P3. **Step 3 — fix every failed case**, each fix landing with unit + regression tests (§11; no fix without a test that failed before it). **Step 4 — update the documentation corpus** (SEC-024: runbooks, EVIDENCE_MATRIX, CLAIMS, INVARIANTS transport tier per SEC-001.3, customer-facing transport-security page, IDENTITY/API docs as touched). **Delivery shape (owner 2026-08-04): AFTER full implementation, `install.py` asks ONE question on fresh install — "Enable TLS/mTLS transport security? [Y/n]" (`--tls=yes|no` for unattended) — yes = the COMPLETE mesh (ingress, nginx→api, all stores, Kafka+ACLs, Vector lanes, sealing transport; seal sidecar implied by the CA seal-gate; boot validator expected fatal=0), no = the declared-plaintext baseline, visibly reported by the posture view. No partial installer toggle ships mid-programme; interim enablement stays override-based.** | High | 🟡 IN PROGRESS — **step 1 COMPLETE 2026-08-09** — enforce wave shipped after soak GO (SEC-007.2 default-deny, SEC-006.3 9092 dropped + inter-broker→MTLS, CH 8123 + valkey 6379 removed, lane-token narrowing proven 4×200/4×401, nginx :8000 dropped on lab, watchdog→https). Residual step-1 tails: shipped-variant nginx :8000 (needs TLS-aware install.py messaging), victoria 8428 direct + api host 8080 rows were already clean. Next = step 2 assurance run (13 phases) |
