...
| 43 | **ITSM Integration Platform — bidirectional, multi-tenant control plane** — 📐 **DESIGN landed** `docs/design/itsm-integration-platform.md` (reconciles the user's control-plane spec with our impl: ~70% of the reliability spine already ships via the PG job queue + incident SoR + per-tenant connectors; new work = inbound webhooks + reconciliation poller + `Provider` interface + per-tenant mapping/state config + `integration_{configs,mappings,events}` tables). Phased P1-P5; credential-encryption staged behind #17; stays stdlib+pgx (PG queue, not Kafka). Today's outbound is one-way (create + auto-resolve; severity→SN impact/urgency; static assignment_group; Jira lacks priority); NO inbound yet. | 📐 design done, build ⏳ |
