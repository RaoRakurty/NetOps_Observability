...
1. **Restore proven for all 3 stores; off-host DR needs infra provisioning.** `restore-drill.sh` proves PG, ClickHouse and OpenSearch each restore with content intact (17/17). The live OpenSearch snapshot repo is now REGISTERED and snapshotting daily (fixed 2026-07-23). What remains is not code: the `BACKUP_REMOTE` off-host push HOOK is built and the Data Protection UI configures it, but a separate-host/object store must be PROVISIONED, and backup.sh scheduled once it is. (§1, §7, BACKUP-FAILURE-DOMAIN.md)
