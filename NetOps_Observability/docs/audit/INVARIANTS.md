...
| Registry storage that cannot serve is distinguishable from an empty registry | ✅ | **BUILD** (tracker 245) — `TestRegistriesStatusReportsTheRealBackend` + the `storageBadge` / Registries UI tests: `GET /api/registries/status` reports configured vs active backend, persistence, availability and reason; the page renders "PostgreSQL · Persistent", "Memory · Ephemeral", "… · Unavailable" or "Unavailable · configured storage: File" from that answer, and shows no badge at all when the posture is unknown. `netops_registry_storage_available` carries the same fact to vmalert |
