...
| `package main` does not grow | 🟡 | **BUILD (ratchet, not a fix)** — `TestFlatPackageMainDoesNotGrow` pins the root package (originally 296 non-test files; **263 as of 2026-07-27** after the chschema/openapi/totp/rca-waves-1+2/vault/vuln+compliance/ratelimit/metricval/noclabel/ticketing extractions); a new file fails the build and must go in a subpackage, and moving files out requires lowering the ceiling in the same commit. Proven to fire both directions. **This does NOT yet satisfy §2**: ~263 files of business logic remain in the entrypoint package, existing files can still grow, and coupling can still increase. Progress + sequence: `docs/design/package-decomposition-plan.md`; see standing gap #8 |
...
8. **`package main` holds most of the business logic, against the repo's own §2.**
   ...
   the decomposition program is UNDERWAY: **263 files remain** and `internal/`
   holds twelve compiler-enforced domains (chschema, openapi, totp, rca
   waves 1+2 — the ~8.3k-LOC report family, vault, vuln, compliance, ratelimit,
   metricval, noclabel, ticketing). For what remains in the root, §13 (no
