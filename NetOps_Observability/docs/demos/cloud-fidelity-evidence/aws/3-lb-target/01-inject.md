# AWS — LB target kill  (scenario `DAWS3`)

Fill during the live capture window. Exact commands: **`../../RUNBOOK.md`**
→ "Scenario 3 — LB target kill" (AWS block).
## What changed
...
- [ ] `03-correlix-signal.png` — `./capture-scenario.sh aws 3-lb-target signal` (or log-lane zoom)
- [ ] `04-correlix-rca.png` — `./capture-scenario.sh aws 3-lb-target rca`
- [ ] `05-recovery.png` — `./capture-scenario.sh aws 3-lb-target recovery` (post-revert + soak)
