# Azure — LB target kill  (scenario `DAZURE3`)

Fill during the live capture window. Exact commands: **`../../RUNBOOK.md`**
→ "Scenario 3 — LB target kill" (Azure block).
## What changed
...
- **Lane / kind:** cloud_lb_log (5xx) + target health
...
- [ ] `03-correlix-signal.png` — `./capture-scenario.sh azure 3-lb-target signal` (or log-lane zoom)
- [ ] `04-correlix-rca.png` — `./capture-scenario.sh azure 3-lb-target rca`
- [ ] `05-recovery.png` — `./capture-scenario.sh azure 3-lb-target recovery` (post-revert + soak)
