# Azure — Host stop  (scenario `DAZURE5`)

Fill during the live capture window. Exact commands: **`../../RUNBOOK.md`**
→ "Scenario 5 — Host stop" (Azure block).
## What changed
...
- **Lane / kind:** cloud_metric / status (power_state)
- **Log Search query:** `cloud_metric AND resource:*app-host-01*`
...
- [ ] `03-correlix-signal.png` — `./capture-scenario.sh azure 5-host-stop signal` (or log-lane zoom)
- [ ] `04-correlix-rca.png` — `./capture-scenario.sh azure 5-host-stop rca`
- [ ] `05-recovery.png` — `./capture-scenario.sh azure 5-host-stop recovery` (post-revert + soak)
