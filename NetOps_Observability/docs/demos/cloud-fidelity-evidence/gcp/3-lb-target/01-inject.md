# GCP — LB target kill  (scenario `DGCP3`)

Fill during the live capture window. Exact commands: **`../../RUNBOOK.md`**
→ "Scenario 3 — LB target kill" (GCP block).
## What changed
...
- [ ] `03-correlix-signal.png` — `./capture-scenario.sh gcp 3-lb-target signal` (or log-lane zoom)
- [ ] `04-correlix-rca.png` — `./capture-scenario.sh gcp 3-lb-target rca`
- [ ] `05-recovery.png` — `./capture-scenario.sh gcp 3-lb-target recovery` (post-revert + soak)
