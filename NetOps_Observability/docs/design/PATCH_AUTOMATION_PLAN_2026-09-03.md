...
| 6 | Container images | `image:` in 8 compose files; `FROM` in 14 Dockerfiles | 55 `image:` lines (47 distinct refs) + 19 `FROM` | yes |
...
and **zero of the 47 distinct image references in the compose files**. Those 47 are Kafka, Postgres,
...
**`compose.offline-images.yml` is a structural hazard.** It is the offline-mirror pull list: 23 image
references, **tag-only, zero digests**, mirroring images that `docker-compose.yml` pins *by digest*.
The two files can drift silently — the offline bundle would then ship a different image than the
online stack runs, with no error anywhere. Renovate will keep the tags current, but tag-parity with
the digest-pinned file is not something it can assert. A small guard test (`tests/`) comparing the two
lists would close it. Flagged, not fixed here.
...
3. **`compose.offline-images.yml` tag/digest parity** with `docker-compose.yml` is unasserted.
