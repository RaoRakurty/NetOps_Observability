...
| Live SSH collect / state battery / config capture | ✅ read-only, 25 devices | ✅ | ✅ | same device ceiling |
...
- **Ceilings at existing chokepoints**: device count at discovery admission (the store
  already has limits), tenant count at `POST /api/tenants`, retention in the ISM/TTL
