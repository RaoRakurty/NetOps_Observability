...
| Unit | **Monitored device** (C4, landed 1feb8fad): a canonical device with ≥1 qualifying collector intentionally enabled; counted once whatever the telemetry mix; discovery/inventory never consume; configured intent, not recent telemetry. Supersedes "enforced at discovery admission". **Wireless controllers and APs count one each** (owner, 2026-09-05, tracker 256): a monitored controller is one device and every monitored access point is one device, so 1 controller + 50 APs = 51. Discovered-only or inventory-only controllers/APs, and any estate whose wireless integration is disabled, count zero; a controller seen by SNMP *and* managed through the integration is one device. Enforced through the same `internal/devmon` definition — the wireless inventory is reported into the device registry by `wireless.DeviceSource`, so there is no second counter. |
...
| Pricing — **APPROVED AS LAUNCH PRICING HYPOTHESES (owner, 2026-09-05)**; order-form only, NEVER in the licence file | Approved figures: **Community $0 permanently, 25 monitored devices, unlimited discovery**; **Team starter $249/month up to 50 monitored devices, then ~$4/device/month from 51 to 250**; **Enterprise ~$6–$9 per monitored device/month, ~$18k–$30k minimum ARR, sales-assisted/contracted**; **MSP pooled ~$3–$6/device/month, ~$24k+ minimum ARR, pooled monitored devices across managed tenants**. These are market-entry hypotheses, not immutable rules: **reviewed once real design-partner and customer conversion data exists**. Design partners: 12-month price protection, 30–40 % expiring launch discount, never a permanently low list. Pricing stays SEPARATE from runtime entitlement semantics — no dollar figure appears in the entitlement engine, the licence packages or a licence file, which carry features, ceilings, customer, expiry and support only (guarded by `TestNoPriceLiteralsInLicenceCode`). Customer-facing copy: `docs-portal/docs/reference/pricing.md`. |
