...
| Flow logs → REJECT faults | ✅ VPC flow logs (S3+CW lanes) | 🔧 **VNet flow logs** `azure_logs.py` storage-blob reader (v4 flowTuples, flowState D → deny rollup); enablement approved 2026-07-15, blocked on interactive az re-auth (SP is read-only by design) | ✅ **Firewall Rules Logging DENIED** rollup (`gcp.poll_log_lanes`, gate `GCP_FIREWALL_LOGS`) — GCP flow logs carry NO deny records; **LIVE-VERIFIED 2026-07-15**: logging enabled on all correlix rules + explicit logged deny-all (65534, behavior-identical to implied deny), real denied IAP probes rendered on the bus as REJECT rollup naming the rule |
