...
| 2 | 🟡 **E2E browser tests: NONE** (no Playwright/Cypress) | High | **Playwright bootstrapped + first flow DONE** — headless-Chromium E2E driving the real SPA against the Vite dev server with the backend mocked at the network boundary (no 19-service stack). `e2e/commandCenter.spec.ts`: auth → Command Center renders → only actionable customer-network incidents survive (single-signal noise + internal-stack #76 filtered) → expand row → evidence ledger. Pinned `@playwright/test@1.60.0`; blocking `e2e` job in frontend-ci. **Cross-tenant UI guard DONE** (`crossTenantGuard.spec.ts`, 4 tests): against a tenant-isolated mock backend — tenant A sees only A's incident (B's device/corr-id absent everywhere incl. expanded evidence), symmetric for B, a scoped operator has NO tenant-switcher (static chip only), and the SPA never even requests another tenant's data. **Path Trace honesty DONE** (`pathTraceHonesty.spec.ts`, 2 tests): drives the real topology canvas (switch mode → pick src/dst) and asserts the provenance chip — a computed path reads "Computed … not a live trace" (never "Measured"), a measured path reads "Measured" (#8 guard, end-to-end). **Investigate/RCA-pin DONE** (`investigateRca.spec.ts`, 2 tests): entering Investigate auto-pins the incident → the verdict banner renders the server-authored verdict + 92% confidence + the TWO named independent witnesses with operator-facing modality labels (no raw tokens); unpin clears it. **Dependency map DONE** (`dependencyMap.spec.ts`): switching to Dependency requests the flow-derived projection and renders its endpoints on the real React Flow canvas (also the only E2E exercising the graph nodes). **E2E total: 11 across 5 specs — covers ALL user-reachable canvas modes (explore/investigate/path_trace/dependency); capacity + change_review are intentionally disabled ("soon") so no flow exists.** Remaining: deeper assertions only |
