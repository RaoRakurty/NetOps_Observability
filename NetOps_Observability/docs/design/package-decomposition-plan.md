...
| **13 NEXT** | `internal/rca` **wave 2** (the report/analysis family) | 13 | ~9500 | high (internal to the family) | **Mapped 2026-07-27, not yet executed.** The 13 rca_* files with ZERO `*server` methods move together: `rca_accounting` `rca_actions` `rca_consistency` `rca_coverage` `rca_impact_provenance` `rca_issue_context` `rca_merge` `rca_postmortem` `rca_report` `rca_report_accounting_view` `rca_report_html` `rca_report_wording` `rca_semantics`. The AST map (symbol → defining file) shows they are densely interlocked with EACH OTHER but thin toward the rest: the real external deps are `ai_labels.go` (4 syms), a few shared types (`Incident`, `Service`, `Region`, `Confidence`), `asString`/`asFloat` (already duplicated in internal/rca once), and a ticketing seam (`evalTicketDecision`/`incidentPolicy`/`corrTicketFacts`). Six rca files STAY (handlers/stores): `rca_report_http` `rca_reports_list` `rca_promotion` `rca_action_items` `rca_path_view` `rca_report_integrity` — they consume the family's exported surface (`rcaReport`, `fmtUTC`, `buildRcaReport`, `renderRcaReportHTML`, …), which therefore needs exporting. Expect 2–3 commits (semantics/wording → report assembly → html/coverage) and per-symbol decisions at the ticketing and ai_labels seams. Beware ident-shadowing false positives in any grep-based map (`sevRank`, `str`, `m` are locals). |
| 14+ | `session`, `oidc`, `tenant`, `seam`, `copilot`, `snmp`, `wireless`, … | — | — | 4–10 | Larger blast radius; take after the rca wave lands. The `jwt` security change (see below) gates the auth tier. |
