...
**Status:** ten domains shipped (`internal/chschema`, `internal/openapi`,
...
`internal/noclabel`, `internal/ticketing`, all 2026-07-27). **263** non-test
...
| 15+ | `verify` (engine+modules, ~1.2k LOC, thin deps), `segment_classifier`, `seam`, `session`, `oidc`, `tenant`, `copilot`, `snmp`, `wireless`, … | — | — | 4–10 | Re-measured 2026-07-27: `verify_engine.go`+`verify_modules.go` pass the screen with a thin seam (`envInt`, `secEnvDuration`, `hostOnly`, `User`); `segment_classifier.go` passes with `tierRank`/`orDefault` (+ possible Region/Service — verify against the compiler, the grep map lies). The big stores (`ticketing_store`, `path_graph_store`, `nms_store`, `wireless_store`) pass the auth screen but need the portintel-style pg-injection treatment. The `jwt` security change (see below) gates the auth tier. |
