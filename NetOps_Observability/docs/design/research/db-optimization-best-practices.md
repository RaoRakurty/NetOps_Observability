...
| `SAMPLE` / approximate aggregations (HLL, t-digest) | Product contract: RCA evidence must be exact. Verified: the codebase currently uses **zero** approximation functions (6 exact quantile/uniqExact call sites, all over small windows). `uniqHLL12`-class functions are acceptable for display-only panels when data volume warrants | A top-N/distinct panel measurably slow over >100M-row windows — swap that panel's aggregate, never evidence paths |
