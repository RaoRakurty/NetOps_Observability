...
| **1c** | ✅ **DONE** Wire stores through the Vault | SNMP creds + integration `webhook_secret` (tenant DEK); notify (SMTP/Twilio/ntfy/Slack-webhook/PagerDuty), OIDC client_secret, LDAP bind_password, TACACS secret (platform DEK, centralized in `secrets_config.go`). Copilot key is **env-only** (never persisted) → phase 3, not 1c. Re-seal pass (`SECRETS_RESEAL`) env reserved; encrypt-on-next-write is the active path. |
