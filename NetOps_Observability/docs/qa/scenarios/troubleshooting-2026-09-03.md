...
| **D-7** | LOW (coverage) | `deployment/docker/gnmic/gnmic.yaml` `ownership-gate` | `device_if_*`, `device_cpu_percent`, `device_temp_celsius` are deleted from the canonical lane as SNMP-owned, and no SNMP profile reaches the SR Linux spines — so **no interface or CPU series exists for any lab device**. Consequences: `interfaces/by-vrf` returns 0 rows (Scenario 2), the Investigation *health* lane is permanently `not_connected` (Scenario 4), and P4-D #4 cannot be demonstrated. Not a bug — the ownership rule is deliberate and documented — but the flip it describes (*"prove parity against the raw lane, remove it from the delete list, withdraw it from the SNMP profile in the same change"*) is the blocker for #4 on this lab. Sub-note: the raw lane carries SRL `…/statistics/*` but **no** `oper-state`/`admin-state` series, so parity is not yet provable for the status families. **RESOLVED 2026-09-05 (tracker 230).** The ownership flip was made as one change: the three patterns came out of the gnmic `ownership-gate` and the same families gained `Owner: "gnmi"` in `src/backend/collectors/profiles.go`, so exactly one transport serves each (device, family). The status families could never be proven against the RAW lane — `prometheus_write` drops string values, which is why no `oper-state`/`admin-state` series existed there — so the enum mapping is proven instead through gnmic's OWN engine (`tests/test_gnmi_correlation_lane.py`, docker-backed) and then live: VictoriaMetrics now holds 92 `device_if_oper_status` series across all seven fabric devices with the IF-MIB numerics (`spine1 ethernet-1/1`=1 up, `ethernet-1/10`=2 down), 42 `device_cpu_percent` and 2 `device_temp_celsius`, all `transport="gnmi"`. Coverage gaps found while flipping were closed in the same change: an Arista OpenConfig CPU subscription (`oc-cpu`, verified live on cEOS) and the SR Linux native `established-transitions` path, both so no device loses a series by being labelled gNMI-capable. `tests/test_gnmi_ownership_contract.py` now fails the build if the two ownership tables ever disagree again. |
