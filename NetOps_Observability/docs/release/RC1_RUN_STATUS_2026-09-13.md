...
| G6 | CODEOWNERS (placeholders only) · pin the 6 unpinned `@v4` actions · licence-gate five reject cases proven | Opus | 🔄 running | Sun 08:37 | Sun 10:37 | engineering-safe; no personnel invented |
| G8 | Decision 8: one fail-closed `release-check` entry point (`scripts/release-gate.py`) with PASS/FAIL/BLOCKED-HUMAN/CI-ONLY table + JSON for the RC1 report | Opus | 🔄 running | Sun 08:37 | Sun 11:37 | does not touch the existing storm-SLO `release-gate` target |
