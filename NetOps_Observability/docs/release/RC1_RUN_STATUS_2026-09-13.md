...
| — | merge agent branches, tracker rows deleted, full gate, push, CI green | Fable+Opus | 🔄 gate run 3 green locally, PUSHED 731d1f48; CI 6/7 green, backend-ci Postgres integration RED (`TestPgUsersStore` keys by username) — fix agent running; local chain gains the pgintegration leg (chain10) | Sun 12:37 | Sun 14:37 | after the first wave lands |
...
| — | customer bundle rebuild from the final commit | Opus | ⏳ queued behind CI green | — | Sun 16:37 | ~210 commits stale today |
| — | lab deploy (.122) + qualify | Fable+Opus | ⏳ queued behind CI green | — | Sun 17:37 | verify served bundle markers |
| — | final RC1 preparation PR merged under the full ruleset → FINAL_RC1_SHA → release-only gates | Fable | ⏳ queued | — | Sun 19:37 | then STOP; the tag is the owner's |
