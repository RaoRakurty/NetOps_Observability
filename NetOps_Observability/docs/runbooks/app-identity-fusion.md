...
vendor logs ─► OpenSearch ─► fusion_worker (Go) ─► CH app_observations/app_identities
                                   │  (best-effort)
                                   ▼
                    netops.app.identities.v1 (Redpanda, via pandaproxy REST)
                                   ▼
...
- **Fusion worker** (vendor logs → observations → identities → CH + topic):
  ...
  (default 120), `REDPANDA_PROXY_URL` (default `http://redpanda:8082`).
...
docker exec -i netops-redpanda-1 rpk topic produce netops.cloud <<EOF
...
EOF
docker exec -i netops-redpanda-1 rpk topic produce netops.app.identities.v1 <<EOF
...
| emit_errors climbing on `/api/appid/fusion/status` | pandaproxy unreachable — check `REDPANDA_PROXY_URL`; emit is best-effort (CH persist still holds), so RCA naming via the topic pauses but data is not lost. |
