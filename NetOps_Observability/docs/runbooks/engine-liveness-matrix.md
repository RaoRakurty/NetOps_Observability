...
| **correlation** (required lanes) | Every REQUIRED lane is *subscribable* — the broker neither refuses nor lacks it. Membership cannot see this: `start()` resolves the whole subscription at once, so one refused lane stops all of them | `corr_required_topic_unavailable{topic,reason}`; `corr_required_topic_unavailable_count` (always published, `0` = healthy) | Labelled gauge only exists while a lane is refused; the `_count` is always there, so `0` is a positive statement | warning | A: `CorrRequiredLaneUnauthorized` **NEW** (the *page* for the outcome is `CorrConsumerNotRunning` above) · `scripts/deploy-qualify.sh` **Q1b** |
