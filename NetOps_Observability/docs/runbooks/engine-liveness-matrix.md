...
| **opensearch** | Cluster not red; documents accepted, not rejected; indexing advancing | `opensearch_cluster_status`, `opensearch_indexing_doc_status{code}`, `opensearch_indexing_index_failed` — **polled by vector, not by a scrape job**, so there is deliberately no `up{job="opensearch"}` (see G1) | `LogIngestRateDegraded` self-baselines over 6 h with a `> 100` floor | **page** | A: `OpenSearchClusterRed`, `OpenSearchFloodStageBlock`, `OpenSearchDocumentsRejected`, `OpenSearchRejectionRatioHigh`, `LogIngestStalled`, `OpenSearchStatsMissing` (the meta-alert for the lane itself disappearing) |
...
| **prober** | Synthetic probes keep arriving at the engine | `corr_ingest_events{counter="probes_received"}` | **None needed, and that is the point:** the prober ticks every 30–60 s regardless of customer traffic, which is what makes a flat line unambiguous | **page**-equivalent | A: `CorrProbeLaneFlatlined` (annotations now name "engine not consuming" first, **NEW**) |
