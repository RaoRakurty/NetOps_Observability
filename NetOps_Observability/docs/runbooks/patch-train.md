...
- `deployment/docker/compose.offline-images.yml` lists the same images **tag-only**. Leave it alone:
  it is generated by `install.py` at offline-install time from `docker-compose.yml`'s own digest pins
  (`docker load` restores tags, not digests; the bundle `SHA256SUMS` carries integrity), and it is
  gitignored. Derived, so it cannot drift — and correctly invisible to both Renovate and the SBOM.
