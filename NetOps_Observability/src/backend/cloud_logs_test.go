...
func TestIndexBaseCloud(t *testing.T) {
	for _, sig := range []string{"cloud", "cloudlogs", "cloudlog"} {
		if got := indexBase(sig); got != "netops-cloudlogs" {
			t.Errorf("indexBase(%q) = %q, want netops-cloudlogs", sig, got)
		}
	}
}
...
func TestTenantIndexPatternCloud(t *testing.T) {
	if got := tenantIndexPattern("cloud", "acme", true); got != "netops-cloudlogs-*" {
		t.Errorf("platform cloud = %q, want netops-cloudlogs-*", got)
	}
	scoped := tenantIndexPattern("cloud", "acme", false)
	...
	for _, sig := range []string{"", "all"} {
		if pat := tenantIndexPattern(sig, "acme", false); containsSub(pat, "cloudlogs") {
			t.Errorf("tenantIndexPattern(%q) must NOT include cloud logs in 'all': %q", sig, pat)
		}
	}
}
