...
//
// The tenant's service dependency graph, derived from OBSERVED cloud flow
// telemetry: the cloud_flow_pair rollups (top-K (src,dst) ACCEPT pairs per scan,
// emitted by all three cloud flow lanes) become volume-weighted talks_to edges,
// and cloud_flow_log REJECT rollups mark an edge as blocked by security rules.
//
// Endpoints resolve to services in trust order:
//   1. the cloud identity map (cloud_identity_mappings — tags/graph, the
//      authoritative answer the tenant itself declared),
//   2. the declared inventory (private/public IPs + ENIs → resource → app).
// An address neither claims stays honestly UNATTRIBUTED: the top-N unresolved
// endpoints (by observed bytes) render as their own bounded nodes; anything
// beyond the bound is dropped AND counted in meta — never silently, never
// unbounded, never guessed.
//
// Honesty rules:
...
//
// Tenant isolation (§3a): both reads carry the caller's tenant_scope SETTINGS
// literal (the corr_signals FORCE row policy enforces it in the database), and
// endpoint resolution uses only the caller's own identity map + inventory
// (principalTenant-scoped stores). Bounded by construction: clamped window,
// GROUP BY with LIMIT, named columns.
...
// chPairRow is one aggregated (src,dst) row off corr_signals (JSONEachRow).
type chPairRow struct {
	Src       string   `json:"src"`
	Dst       string   `json:"dst"`
	Bytes     float64  `json:"bytes"`
	Obs       int      `json:"obs"`
...
// cloudPairSQL aggregates the window's cloud_flow_pair signals per (src,dst).
// Each pair signal is one scan cycle's NEW flow records (offset-tracked
// producers), so summing across rows sums disjoint observation windows.
func cloudPairSQL(windowHours, limit int, scope string) string {
	return fmt.Sprintf(`
...
// cloudRejectPairSQL aggregates the window's REJECT evidence per (src,dst) —
...
func cloudRejectPairSQL(windowHours, limit int, scope string) string {
	return fmt.Sprintf(`
SELECT JSONExtractString(attrs,'srcaddr') AS src,
       JSONExtractString(attrs,'dstaddr') AS dst,
       0                                  AS bytes,
...
// ── pure graph builder (unit-tested in cloud_service_map_test.go) ─────────────
...
func buildServiceMap(pairs, rejects []chPairRow, resolve func(string) (string, bool), maxUnattributed int) serviceMapGraph {
	if resolve == nil {
	...
	pairSignals := 0
	fold := func(p chPairRow, blocked bool) {
		if serviceMapNoise(p.Src, p.Dst) {
			return
		}
