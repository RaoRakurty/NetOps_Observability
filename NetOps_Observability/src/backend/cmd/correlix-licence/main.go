...
//	correlix-licence keys    [--release-check]
//
...
const usage = `correlix-licence — issue and inspect Correlix licence files
  ...
  keys [--release-check]
      Print the public signing keys this build trusts: id, role, purpose and
      the ceremony note. --release-check exits 1 unless a PRODUCTION-purpose key
      is embedded — the guard that stops a release being cut while the lab key
      is the only thing the binary trusts (tracker 259). See
      docs/runbooks/licence-signing-ceremony.md.
`
...
func cmdKeygen(args []string, out *os.File) error {
	...
	// The pasted line must carry a PURPOSE: keys.go refuses to build a key set
	// with an unknown one, and ReleaseReady reads it. `production` is only
	// honest for a key generated by the ceremony in
	// docs/runbooks/licence-signing-ceremony.md — anything generated on a
	// developer or lab host is `PurposeLab`, whatever it is going to be used for.
	fmt.Fprintf(out, "\t{base64: %q, role: RoleCurrent, purpose: PurposeLab /* PurposeProduction only if this came from the ceremony */, note: \"...ceremony...\"},\n", kp.PublicBase64())
	return nil
}
...
// cmdKeys prints the trusted public key set, and optionally refuses.
//
// The refusal is the point. A licence signed with the lab key verifies exactly
// like one signed with the production key — so the only place a release can be
// told apart from a lab build is the set of keys it carries. `--release-check`
// asks licence.ReleaseReady() that question and exits non-zero while the answer
// is no, which is what a release step (and a human following
// docs/runbooks/licence-signing-ceremony.md §10) needs.
func cmdKeys(args []string, out *os.File) error {
	fs := flag.NewFlagSet("keys", flag.ContinueOnError)
	releaseCheck := fs.Bool("release-check", false, "exit 1 unless a production-purpose signing key is embedded")
	if err := fs.Parse(args); err != nil {
		return err
	}
	if fs.NArg() != 0 {
		return errors.New("keys: takes no positional arguments")
	}
	keys := licence.EmbeddedKeys()
	fmt.Fprintf(out, "embedded signing keys (%d):\n", len(keys))
	for _, k := range keys {
		purpose := k.Purpose
		if purpose == "" {
			purpose = "unlabelled"
		}
		fmt.Fprintf(out, "  %s  role=%-8s purpose=%-10s %s\n", k.ID, k.Role, purpose, k.Base64)
		if k.Note != "" {
			fmt.Fprintf(out, "      %s\n", k.Note)
		}
	}
	...
	if ready == nil {
		fmt.Fprintln(out, "release-ready: yes (a production-purpose key is embedded)")
		return nil
	}
	fmt.Fprintf(out, "release-ready: NO — %v\n", ready)
	if *releaseCheck {
		return ready
	}
	return nil
