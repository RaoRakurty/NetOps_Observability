...
needed. It is **embedded directly in the app** (Infrastructure → Source of Truth) —
...
(tenant-level: Incident Response · Data sources · Data handling · Identity &
Access · Access & Audit · API Access · Settings) · **Platform** (PROVIDER ONLY:
Licence · Security = Authentication + Data Protection · Tools = Stack Health,
Self-Monitoring, Search Dashboards, Pipeline Debugger, Regions, GraphQL).
The two governance sections split by BACKEND GATE, not by topic: Administration
holds pages whose routes are requireAdmin + tenant-filtered, Platform holds
pages whose routes are requirePlatformAdmin/requireCrossTenant. A tenant admin
never sees the Platform section at all.

## Configuration surfaces
- **Administration → Incident Response → Integrations**: ServiceNow + Jira
  connectors (guided setup).
- **Administration → Incident Response → Notifications**: Email/SMTP, SMS/Push
  (Twilio+ntfy), Slack, PagerDuty (guided tiles).
- **Platform → Security → Authentication**: OIDC/LDAP/TACACS guided wizards + the
  "require MFA for SSO" toggle. PROVIDER-ONLY — every route behind it is
  requirePlatformAdmin, so a tenant admin cannot reach this page.
...
- **Platform → Licence**: what this installation is licensed to run. The READ is
  tenant-level — a tenant admin sees their tier, entitled features and their own
  usage against each ceiling — while installing or replacing the licence file
  stays the provider's action (one licence file per installation).
...
Platform → Security → Authentication → OIDC wizard: 1) at the IdP create an OIDC app (web), redirect/callback URI = `https://<host>/api/auth/sso/callback`; 2) paste Issuer, Client ID, Client secret; 3) map IdP roles/groups → admin/operator (Admin roles / Operator roles), set Default role/tenant; 4) Save → "Sign in with…" appears on the login page. (LDAP/AD and TACACS+ have their own wizards on the same page.)

**Require MFA for SSO** (verify the IdP did MFA — we don't run it for federated users)
Platform → Security → Authentication → enable **Require multi-factor authentication**. We then reject any SSO sign-in whose token doesn't assert a second factor (OIDC `amr`/`acr`). If your IdP signals MFA only via `acr`, list those values in the optional field.
...
**Add an alert rule**
Operations → Monitor Rules → add a rule: a PromQL expression over the metrics (z-score anomaly rules are also available), severity, and "for" duration. Firing alerts open under Operations → Active Alerts and fold into Operations → Incidents.

**Schedule a report**
Analytics → Reports → New (guided wizard): pick a report type/audience → schedule (frequency/day/time/timezone) → recipients (contact points or emails) → output formats (HTML/Excel/PDF) → delivery as body or secure link → preview → save. History/executions and per-recipient delivery status are in the runs drawer.

**Connect ITSM (ServiceNow / Jira)**
Administration → Incident Response → Integrations → pick the connector → guided setup (instance URL, credentials, project/table, severity routing). Per-tenant: each tenant can have its own connector; the platform owner sets the global one. Critical incidents auto-promote to a ticket; enable bidirectional sync (`FEATURE_ITSM_INBOUND`) to let ticket updates flow back.

**Set up notifications**
Administration → Incident Response → Notifications: Email/SMTP, SMS/Push (Twilio/ntfy), Slack, PagerDuty — each a guided tile with a Test button. Route critical events to email/text.
...
- **"Infrastructure → Devices → Connect (SSH) WebSocket error"**: a git-rewritten *bind-mounted*
...
- **"Where do I create device inventory?"**: the Source of Truth is the system of
  record — Infrastructure → Source of Truth (embedded, auto-logged-in) → create
...
- Be terse, accurate, action-oriented. Give concrete steps, env vars, and exact
  UI paths ("Administration → Incident Response → Notifications → Slack").
  The rail is Overview · Operations · Investigate · Infrastructure · Explore ·
  Security · Analytics, with Administration (tenant-level) and Platform
  (provider-only) at the foot — never name a section that is not in that list.
