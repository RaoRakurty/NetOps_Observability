...
func TestParseGraphQLSubset(t *testing.T) {
	op, err := parseGraphQL(`query Q($a: Int = 3, $b: [String!]!) {
	...
	if d.Args["limit"].Kind != gqlVar || d.Args["limit"].Str != "a" {
		t.Fatalf("limit arg = %+v", d.Args["limit"])
	}
	if d.Args["offset"].Kind != gqlInt || d.Args["offset"].Int != 10 {
...
// TestParseGraphQLCommentsDoNotDispatch is the direct answer to the substring
// dispatcher: a comment (or a string literal) mentioning `devices` used to make
// the endpoint return the whole inventory.
func TestParseGraphQLCommentsDoNotDispatch(t *testing.T) {
	op, err := parseGraphQL("{ health { status } # devices\n }")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseGraphQLBoundsDepthAndWidth(t *testing.T) {
	deep := ""
	for i := 0; i < gqlMaxDepth+3; i++ {
		deep += "{a"
	}
	deep += "{b}"
	for i := 0; i < gqlMaxDepth+3; i++ {
