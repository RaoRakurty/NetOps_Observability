...
func evaluateFixture(t *testing.T, platform, raw string) []secfindings.Finding {
	t.Helper()
	dev := Device{ID: "d1", Hostname: "lab", Platform: platform, TenantID: "acme"}
	eng := NewEngine(DefaultCatalog(), MemConfigSource{"d1": raw}, MemSeamResolver{}, WithClock(fixedClock()))
...
func TestFabricPlatformsResolveToTheirOwnDialect(t *testing.T) {
	cases := []struct {
		...
		{platformEOS, VendorArista, "Arista EOS"},
		{"arista", VendorArista, "Arista EOS"},
		{"ceos", VendorArista, "Arista EOS"},
		{platformSRLinux, VendorSRLinux, "Nokia SR Linux"},
		{"srlinux", VendorSRLinux, "Nokia SR Linux"},
		{"Nokia SR OS 22", VendorNokia, "Nokia SR OS"},
	}
	for _, c := range cases {
		if got := VendorFromPlatform(c.platform); got != c.want {
			t.Errorf("VendorFromPlatform(%q) = %q, want %q", c.platform, got, c.want)
		}
		if got := DisplayVendor(c.want); got != c.display {
...
func TestFabricUnknownWhenConfigUnavailable(t *testing.T) {
	for _, platform := range []string{platformEOS, platformSRLinux} {
		dev := Device{ID: "d1", Hostname: "lab", Platform: platform, TenantID: "acme"}
		eng := NewEngine(DefaultCatalog(), MemConfigSource{}, MemSeamResolver{}, WithClock(fixedClock()))
		fs, err := eng.Evaluate(context.Background(), dev)
		if err != nil {
			t.Fatalf("evaluate: %v", err)
		}
...
func TestFabricDetectionsAreNotFooledByNearMisses(t *testing.T) {
	...
	httpsOnly := NewConfig(VendorSRLinux, "set / system json-rpc-server network-instance mgmt https admin-state enable\n")
	if r := srlJSONRPCPlaintext(httpsOnly); r.Tripped {
		t.Errorf("srlJSONRPCPlaintext tripped on an HTTPS-only listener: %q", r.Evidence)
	}
	eosHTTPS := NewConfig(VendorArista, "management api http-commands\n   no shutdown\n   protocol https\n")
	...
	eosTelnetShut := NewConfig(VendorArista, "management telnet\n   shutdown\n")
	...
	mixed := NewConfig(VendorSRLinux, strings.Join([]string{
		"set / system grpc-server mgmt admin-state enable",
	...
	hashed := NewConfig(VendorSRLinux, "set / system aaa authentication admin-user password $y$REDACTED\n")
	if r := srlWeakLocalSecret(hashed); r.Tripped {
		t.Errorf("srlWeakLocalSecret tripped on a hashed password: %q", r.Evidence)
	}
	clear := NewConfig(VendorSRLinux, "set / system aaa authentication admin-user password NokiaSrl1\n")
