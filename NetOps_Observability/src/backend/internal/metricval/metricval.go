...
// nonFinite counts samples rejected as non-finite. Write-only atomic, no
// configuration — the same shape as package main's jsonEncodeFailures; exposed
// read-only via NonFinite() for the operator metrics endpoint.
var nonFinite atomic.Uint64

// NonFinite reports the number of samples rejected as non-finite since start.
func NonFinite() uint64 { return nonFinite.Load() }

// Parse parses a metric-store value string. ok is false when the
...
func Parse(s string) (v float64, ok bool) {
...
// FiniteOrZero is Parse for the call sites whose response field is a
...
// Sanitize is the last-line guard for a float that was COMPUTED rather than
