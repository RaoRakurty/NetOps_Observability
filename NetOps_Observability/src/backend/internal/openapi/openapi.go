...
var apiRoutes = []apiRoute{
	...
	// Elevation providers: the caller's own time-bound grant.
	{"GET", "/api/auth/elevation", "Auth", "The elevated access the CALLER currently holds: which role, over what scope, why, and when it expires. Self-scoped — it answers about the caller and nobody else"},
	{"DELETE", "/api/auth/elevation", "Auth", "End the caller's own elevated access immediately, before it expires on its own (self-scoped, audited)"},
	// Per-tenant sign-in and SSO URLs (design §6.1, tracker 276).
	{"GET", "/api/auth/locator", "Auth", "Resolve a per-tenant sign-in URL (/t/{slug} or /org/{org_id}) to a candidate realm and arm the signed candidate cookie. Public; an unknown or inactive locator answers the same empty result as a known one, with no information leak"},
	{"GET", "/api/auth/sso/tenant-idp", "Auth", "This tenant's own identity-provider connections, with the sign-in and per-tenant callback URLs to hand an IdP team (administration:admin, own tenant only)"},
	{"GET", "/api/auth/sso/tenant-idp/{alias}", "Auth", "One of this tenant's connections. Another tenant's alias answers the same 404 as an unknown one (administration:admin)"},
	{"PUT", "/api/auth/sso/tenant-idp/{alias}", "Auth", "Create or edit a connection in the caller's OWN tenant. The owning tenant is stamped from the token and cannot be named in the body (administration:admin)"},
	{"DELETE", "/api/auth/sso/tenant-idp/{alias}", "Auth", "Delete one of this tenant's connections (administration:admin)"},
