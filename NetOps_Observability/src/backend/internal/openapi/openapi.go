...
var apiRoutes = []apiRoute{
	...
	{"PUT", "/api/bgp/alerts/config", "BGP", "Replace the caller's alert policy (infrastructure:write; the owner is stamped from the token, never the body). An empty expected-origin set stores NO baseline: each pass compares against its own dominant origin, so only a minority unexpected origin is detectable and a fully propagated origin change classifies clean. An empty upstream set disables the route-leak heuristic rather than guessing a transit set"},
