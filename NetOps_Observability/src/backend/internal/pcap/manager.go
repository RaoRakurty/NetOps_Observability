...
// a packet capture is real frames off a customer's production interface and the
// platform owner's cross-tenant reach is not automatically a right to read them.
...
type Principal struct {
	...
	// device here, so it lists no capture, reads no status and streams no
	// packet — the same answer logs, flows, metrics, igpmon and the BMP feed
	// give.
	Deny bool
...
// Admits reports whether this principal may see captures owned by `owner`. It is
