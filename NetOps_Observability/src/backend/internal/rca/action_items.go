...
// NewActionItemStore opens the register at path ("" = memory-only, tests). A
...
func NewActionItemStore(path string) *ActionItemStore {
	...
	b, err := loadRegister(path, "rca action items")
	if err != nil {
		s.unreadable = err
		return s
	}
	...
	if uerr := json.Unmarshal(b, &m); uerr != nil {
		s.unreadable = unparsedRegister("rca action items", uerr)
		return s
	}
	s.m = m
	return s
...
// case with no remediation work.
func (s *ActionItemStore) Unavailable() error {
	if s == nil {
		return nil
	}
	s.mu.RLock()
