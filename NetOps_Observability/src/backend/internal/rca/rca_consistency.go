...
func validateRcaReport(rep *Report, now time.Time) rcaReportQuality {
	...
	for _, o := range acc.AnomalyObservers {
		if o.Kind == KindLogicalVantage && rcaObserverRegistry.IsNeverVantage(o.ObserverID) {
...
func applyReportQualityGate(rep *Report, now time.Time) {
