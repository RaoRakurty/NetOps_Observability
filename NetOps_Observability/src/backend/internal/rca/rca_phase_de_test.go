...
func TestPhaseD_TenantIsolation(t *testing.T) {
	...
	reg := NewObserverRegistry(map[string]map[string]ObserverKind{
		"tenant-a": {"site-probe-9": KindLogicalVantage},
	})
	f := ObserverFacts{ObserverID: "site-probe-9", Modality: "active_probe", ProbeAuthority: "low"}
	if got := reg.Classify("tenant-a", f); got != KindLogicalVantage {
		t.Errorf("tenant-a classify = %q, want logical_vantage", got)
	}
	if got := reg.Classify("tenant-b", f); got == KindLogicalVantage {
...
func baseValidReport(t *testing.T) Report {
	t.Helper()
...
func gateErrorCodes(rep Report) map[string]bool {
...
func TestPhaseE_Blockers(t *testing.T) {
	...
	tests := []struct {
		...
		{"case_linked>total", func(r *Report) {
		...
		{"anomalous>case_linked", func(r *Report) {
		...
		{"failed>executions", func(r *Report) {
		...
		{"independent>observers", func(r *Report) {
		...
		{"verdict_gate_disagreement", func(r *Report) {
		...
		{"api_as_vantage", func(r *Report) {
			r.accounting.AnomalyObservers = []observerRecord{{ObserverID: "api", Kind: KindLogicalVantage}}
		}, "api_or_collector_as_vantage"},
		{"full_below_threshold", func(r *Report) {
			a := partial
			a.CoverageRatio = 0.4
			r.Coverage = []rcaEvidenceLane{{Class: "active_probe", Coverage: "full", State: "normal", Assessment: &a}}
		}, "coverage_full_below_threshold"},
		{"normal_with_material_gap", func(r *Report) {
		...
		{"point_in_time_as_full", func(r *Report) {
			a := full
		...
		{"impact_none_detected_without_coverage", func(r *Report) {
		...
		{"real_user_none_detected_without_eligible_coverage", func(r *Report) {
			r.States.ImpactRealUser = "none_detected"
			a := partial
			r.Coverage = []rcaEvidenceLane{{Class: "passive_flow", Assessment: &a}}
		}, "real_user_none_detected_without_eligible_coverage"},
		{"synthetic_none_detected_without_eligible_coverage", func(r *Report) {
			r.States.ImpactSynthetic = "none_detected"
			a := partial
...
func buildP027379Report(t *testing.T) Report {
	t.Helper()
...
func p027379AfterTables(rep Report) string {
