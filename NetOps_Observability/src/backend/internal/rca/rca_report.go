...
type Report struct {
	...
	Integrity *ReportIntegrity `json:"integrity,omitempty"`
	...
	States ReportStates   `json:"states"`
	...
	Phases []IncidentPhase `json:"phases,omitempty"`
	...
	FaultLocalization FaultLocalization `json:"fault_localization"`
	...
	PathAttribution *PathAttribution `json:"path_attribution,omitempty"`
	...
	// layer for tenant-keyed store writes (revision register). Kept unexported
	// ON PURPOSE: an unexported field is structurally unserializable, so no
	// JSON tag mistake can ever leak it. The integrator reaches it through
	// SetOwnerTenant/OwnerTenant.
	ownerTenant string
...
func (r *Report) evidenceAccounting() (EvidenceAccounting, error) {
	return r.accounting, r.accountingErr
}

// SetOwnerTenant stamps the object's owning tenant for tenant-keyed store
// writes. The field stays unexported so it is structurally unserializable.
func (r *Report) SetOwnerTenant(t string) { r.ownerTenant = t }

// OwnerTenant returns the owning tenant stamped by SetOwnerTenant.
func (r *Report) OwnerTenant() string { return r.ownerTenant }

type SpineHopView struct {
...
type TopologyView struct {
	...
	Hops               []SpineHopView `json:"hops,omitempty"`
...
func StampTopologyTemporalRole(rep *Report) {
	t := &rep.Topology
...
type ReportStates struct {
	...
	RecoveryComponent RecoveryScopeState `json:"recovery_component"`
	RecoveryService   RecoveryScopeState `json:"recovery_service"`
...
// FaultLocalization (P1.3/P1.4): where the evidence converges — a seam,
...
// RootCause may claim identification ONLY when a causal mechanism AND a
...
// AtAGlance — the owner-mandated first section (#113 point 2): where · what
// possibly happened · possible owner(s). The causality path renders alongside it
// from Report.Topology (broken areas red, rcaPathGraphSVG). Wording rules:
...
// in labels.ts.
var OwnerTeam = map[string]string{
...
func BuildReport(in ReportInput) Report {
	...
	incident := "active"
	mergedLifecycle, isMerged := MergeIncidentState(state)
	switch {
	case isMerged:
	...
	loc := FaultLocalization{Statement: "The fault has not been localized to a specific domain or object."}
	...
	root := RootCause{Identified: false, Statement: "Root cause has not been identified."}
