...
func buildTestReport(t *testing.T, meta map[string]any, sigs []map[string]any) Report {
	t.Helper()
...
func TestRcaReportCarriesCascade(t *testing.T) {
	...
	htmlBytes, err := RenderReportHTML(rep)
	if err != nil {
		t.Fatal(err)
	}
...
func TestManagementSummaryTrimOverflow(t *testing.T) {
	...
	rep := Report{Summary: rcaReportSummaries{Management: "short"}, mgmtTrimmed: true,
		States: ReportStates{Incident: "active", Recovery: "not_observed"}}
