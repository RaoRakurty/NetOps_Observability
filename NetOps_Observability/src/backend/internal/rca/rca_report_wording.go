...
func buildHypothesesView(hb rcaHypBlob, kindCounts map[string]int, kindObservers map[string]map[string]bool) []rcaHypothesis {
	...
	for i, h := range hb.Ranking.Hypotheses {
		...
		hy := rcaHypothesis{
			...
			Owner:         OwnerTeam[h.Verdict.Owner],
		}
...
func buildOwnership(analysis string, faultLocalized bool, serviceClassification string, hb rcaHypBlob, sig rcaSignalSummary, kindCounts map[string]int) Ownership {
	own := Ownership{
		TriageOwner:      "NOC",
	...
	for _, h := range hb.Ranking.Hypotheses {
		...
		team := OwnerTeam[h.Verdict.Owner]
		if team == "" || seen[team] {
			continue
		}
		seen[team] = true
	...
	if len(hb.Ranking.Hypotheses) > 0 && (analysis == "confirmed" || analysis == "probable") {
		...
		team := OwnerTeam[top.Verdict.Owner]
		if team != "" {
...
func buildAtAGlance(loc FaultLocalization, scope rcaReportScope, hyps []rcaHypothesis, own Ownership, root RootCause, analysis string) AtAGlance {
	g := AtAGlance{}
...
func rcaSeverityIncidentBasis(sevIncident string, validation bool, impactSyn, impactRU, analysis string, observers, anomLanes int, ra RecoveryAssessment) string {
	if validation {
...
func buildManagementSummary(problemNoun string, scope rcaReportScope, times rcaReportTimes, incident, analysis, impact, impactSyn, impactRealUser, monitoring string, decision rcaDecision, sig rcaSignalSummary, monitorWindow time.Duration, ra RecoveryAssessment, merge *rcaIncidentMerge) (string, bool) {
...
func buildNocQuickRead(incident, recovery, analysis, impact, impactSyn, impactRU, ticket, monitoring string, times rcaReportTimes, scope rcaReportScope, sig rcaSignalSummary, coverage []rcaEvidenceLane, own Ownership, actions []rcaAction, ra RecoveryAssessment, merge *rcaIncidentMerge) []rcaKV {
	var kv []rcaKV
