...
func TestNormalizeExportFormat(t *testing.T) {
	...
	for in, want := range cases {
		if got := normalizeExportFormat(in); got != want {
			t.Errorf("normalizeExportFormat(%q) = %q, want %q", in, got, want)
		}
	}
}
...
func TestFlattenLogSource(t *testing.T) {
	...
	row := flattenLogSource(json.RawMessage(flow), "netops-flows-2026.06.03")
	...
	row = flattenLogSource(json.RawMessage(app), "netops-applogs-2026.06.03")
	if row[1] != "api" {
	...
	row = flattenLogSource(json.RawMessage(sys), "netops-syslog-2026.06.03")
	...
	row = flattenLogSource(json.RawMessage(raw), "i")
...
func TestBuildExportSearchBody_TenantScoping(t *testing.T) {
	...
	cross := logExportSpec{Cross: true}
	js := mustJSON(t, buildExportSearchBody(cross, t0, t1, 1000, nil))
	...
	scoped := logExportSpec{DeviceKeys: []string{"router-01"}, DeviceAddrs: []string{"10.0.0.1"}}
	js = mustJSON(t, buildExportSearchBody(scoped, t0, t1, 1000, nil))
	...
	empty := logExportSpec{Cross: false}
	js = mustJSON(t, buildExportSearchBody(empty, t0, t1, 1000, nil))
	...
	js = mustJSON(t, buildExportSearchBody(cross, t0, t1, 1000, []any{12345, "abc"}))
...
func TestEncodeExport(t *testing.T) {
	...
	art, err := encodeExport(ctx, "csv", cols, data)
	...
	art, _ = encodeExport(ctx, "ndjson", cols, data)
	...
	art, _ = encodeExport(ctx, "json", cols, data)
	...
	art, err = encodeExport(ctx, "xlsx", cols, data)
	...
	art, _ = encodeExport(ctx, "ndjson", cols, exportData{rows: data.rows})
