...
func TestPhase2ArtifactClassPreserved(t *testing.T) {
	...
	html, err := renderRcaReportHTML(val)
	if err != nil {
		t.Fatalf("render: %v", err)
	}
	doc := string(html)
	...
	html2, err := renderRcaReportHTML(op)
	if err != nil {
		t.Fatalf("render: %v", err)
	}
...
func TestPhase2NoEngineJargonInDocument(t *testing.T) {
	for name, rep := range map[string]rcaReport{
...
func TestPhase2FixtureDocumentSnapshots(t *testing.T) {
	op := buildFixtureP3335CF(t)
	opHTML, err := renderRcaReportHTML(op)
	if err != nil {
	...
	val := buildFixturePD96E4C(t)
	valHTML, err := renderRcaReportHTML(val)
	if err != nil {
