...
func TestErrorIsNotConflatedWithABenignState(t *testing.T) {
	...
	// predate the guard and were simply invisible to it. Tracked as tracker 306,
	// which carries the per-caller work; SHRINK-ONLY, exactly like the path
	// baseline below.
	baselineFn := map[string]bool{
		"collectors/redis.go#FetchIfAddrMap":        true, // 306
		"collectors/redis.go#FetchRoutingDirection": true, // 306
		"collectors/redis.go#FetchIfIndexMap":       true, // 306
		"collectors/redis.go#FetchWANCircuits":      true, // 306
	}
