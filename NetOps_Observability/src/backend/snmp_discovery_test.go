...
func TestValidateDiscoveryRangesRefusesBadInput(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			if _, _, err := validateDiscoveryRanges(c.ranges, false); err == nil || !strings.Contains(err.Error(), c.errHas) {
...
func TestValidateDiscoveryRangesNonPrivateAcknowledgment(t *testing.T) {
	...
	if _, _, err := validateDiscoveryRanges([]string{"172.40.40.0/24"}, false); err == nil || !strings.Contains(err.Error(), "not private") {
		t.Fatalf("non-private without acknowledgment must be refused, got %v", err)
	}
	clean, total, err := validateDiscoveryRanges([]string{"172.40.40.0/24"}, true)
	...
	for _, r := range []string{"127.0.0.0/24", "169.254.1.0/24", "224.0.0.0/24", "0.0.0.0/24"} {
		if _, _, err := validateDiscoveryRanges([]string{r}, true); err == nil {
...
func TestValidateDiscoveryRangesAcceptsAndNormalizes(t *testing.T) {
	clean, total, err := validateDiscoveryRanges([]string{" 10.20.0.1/24 ", "", "192.168.5.0/26"}, false)
	if err != nil {
...
func fakeProbe(alive map[string]string) discoveryProbe {
...
func TestSNMPSourceDiscoversOnlyUnknownHosts(t *testing.T) {
	cfg := discoveryScanConfig{Enabled: true, Ranges: []string{"10.20.0.0/29"}}
	known := []models.Device{{ID: "existing", Address: "10.20.0.2"}}
	src := NewSNMPSource(
		func() discoveryScanConfig { return cfg },
		func() []models.Device { return known },
	)
	src.probe = fakeProbe(map[string]string{
...
func TestSNMPSourceDisabledAndOversizedAreSafe(t *testing.T) {
	...
	var probed bool
	src := NewSNMPSource(
		func() discoveryScanConfig {
			return discoveryScanConfig{Enabled: false, Ranges: []string{"10.20.0.0/29"}}
		},
		nil,
	)
	src.probe = func(context.Context, string, string) (string, string, string, bool) {
		probed = true
		return "", "", "", false
	}
	...
	src2 := NewSNMPSource(
		func() discoveryScanConfig { return discoveryScanConfig{Enabled: true, Ranges: []string{"10.0.0.0/8"}} },
		nil,
	)
	src2.probe = func(context.Context, string, string) (string, string, string, bool) {
...
func TestSNMPSourceTriesCommunityList(t *testing.T) {
	...
	cfg := discoveryScanConfig{Enabled: true, Ranges: []string{"10.20.0.0/31"}, Community: "arista-public, srl-public"}
	src := NewSNMPSource(func() discoveryScanConfig { return cfg }, nil)
	src.probe = func(_ context.Context, addr, community string) (string, string, string, bool) {
...
func TestDiscoveryBoundsContract(t *testing.T) {
	if discoveryMaxHosts > 4096 {
		t.Fatalf("discoveryMaxHosts raised to %d — review the scan-abuse implications before widening", discoveryMaxHosts)
	}
	if discoveryCooldown < 30*time.Second {
		t.Fatalf("discoveryCooldown lowered to %v — refresh endpoint becomes a scan-amplification lever", discoveryCooldown)
	}
}
