...
def test_builtin_catalog_validates():
    cat = builtin_catalog()
    assert len(cat.enabled_templates()) == 91  # +3 NMS controller-intel (P4b); +1 saas-experience-degraded; +1 cloud app-dependency-down; +1 cloud ipsec-tunnel-down; +1 middle-mile ipsec-underlay-down; +4 cloud edge-dependency attribution (Wave 3 #9: lb-5xx / waf-block / firewall-reject / dns-failure)
