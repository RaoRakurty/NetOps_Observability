...
// Administration → Data Collection group (beside the other ingest plumbing) and
...
describe("Telemetry coverage nav placement", () => {
  it("sits in the Administration → Data Collection group", () => {
    expect(leaf).toBeDefined();
    expect(leaf!.label).toBe("Telemetry Coverage");
    expect(leaf!.group).toBe("Data Collection");
  });

  it("is the last Data Collection item, after the shaping/handling pair", () => {
    const dataCollection = (admin.children ?? []).filter((l) => l.group === "Data Collection").map((l) => l.id);
    expect(dataCollection).toEqual([
      "datasources", "snmp", "sensors", "processors", "sensitive-data-access", "telemetry-coverage",
    ]);
  });
